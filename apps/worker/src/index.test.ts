...
describe("finding delivery", () => {
  ...
  test("prepareFindingDelivery formats evidence and recommendation as fenced markdown blocks", () => {
    const delivery = prepareFindingDelivery(
      [
        {
          ...baseFinding,
          findingId: "markdown-shape",
          ruleId: "rule/markdown",
          filePath: "src/a.ts",
          line: 7,
          evidence: "const message = `inline`;",
          recommendation: "Prefer explicit typing for this expression.",
          confidence: 0.92,
        },
      ],
      {
    ...
    const commentBody = delivery.comments[0]!.body;
    expect(commentBody).toContain("### Mergewise Finding");
    expect(commentBody).toContain("- Patch Policy: `manual-only`");
    expect(commentBody).toContain("**Evidence**");
    expect(commentBody).toContain("```ts");
    expect(commentBody).toContain("const message = `inline`;");
    expect(commentBody).toContain("**Recommendation**");
    expect(commentBody).toContain("```text");
    expect(commentBody).toContain("Prefer explicit typing for this expression.");
    expect(commentBody).toContain("\"patchSuggestionPolicy\": \"manual-only\"");
  });

  test("prepareFindingDelivery includes patch preview markdown for safe patch findings", () => {
    const delivery = prepareFindingDelivery(
      [
        {
          ...baseFinding,
          findingId: "safe-patch",
          ruleId: "rule/safe",
          filePath: "src/patch.ts",
          line: 14,
          evidence: "if (enabled) run();",
          recommendation: "Guard the call with a truthy check.",
          confidence: 0.94,
          patchSuggestionPolicy: "safe-patch" as const,
          patchPreview: {
            hunkHeader: "@@ -14,1 +14,1 @@",
            removedLines: ["if (enabled) run();"],
            addedLines: ["if (enabled === true) run();"],
          },
        },
      ],
      {
    ...
    expect(delivery.comments).toHaveLength(1);
    const commentBody = delivery.comments[0]!.body;
    expect(commentBody).toContain("- Patch Policy: `safe-patch`");
    expect(commentBody).toContain("**Suggested Patch**");
    expect(commentBody).toContain("```diff");
    expect(commentBody).toContain("@@ -14,1 +14,1 @@");
    expect(commentBody).toContain("-if (enabled) run();");
    expect(commentBody).toContain("+if (enabled === true) run();");
    expect(commentBody).toContain("\"patchSuggestionPolicy\": \"safe-patch\"");
  });
